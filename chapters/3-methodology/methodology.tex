...
Regarding the implementation of the Orbital Third person Camera, we chose to use the \emph{Cinemachine} Unity Package due to its comprehensive API and components that include functionality for multiple types of cameras, including Orbital third person Cameras. Another useful trait is that APIs provided by Cinemachine enable defining parametric curves to calculate vertical and horizontal positioning of the camera when rotated by the player or by external components. 

%     * Algorithmic motion that can simulate cinematic behavior with camera shake effect and framing corrections
Cinemachine also provides systems for , composing and blending multiple cameras, which satisfies our requirement of handling multiple camera modes by facilitating the transition between shots with a behavior-controlled signal system. Furthermore, Cinemachine also defines algorithms for procedural generation of motion and framing, which is useful to create screen shake effects with high fidelity when the player is attacked by an enemy, or when the camera needs to track the movement of high velocity targets.
