...
\begin{table}[!ht]
    \begin{center}
      ...
      \begin{tabular}{ w{c}{6em} m{27em} } % alignments and column size
        ...
        \makecell[c]{Completion\\Time} & The amount of time, with value $v \in \mathbb{R}$ and represented in minutes, which elapses between when the player is given control of their character after a given level starts, and when the player successfully reaches the end of such level. \\
...
% 70% thought that the enemies in the second version were smarted, with 15% being unable to determine
...
\begin{table}
    \begin{center}
      ...
      \begin{tabular}{ >{\small}w{c}{6em} >{\footnotesize}w{l}{28em} } % alignments and column size
        ...
        \makecell*[{{p{33em}}}]
        {
            \textbullet\space Globally lower adjustment targets in the Dynamic difficulty version for both Groups. Might show issue with balancing and target thresholds. \\ % (global) 
            \textbullet\space Significantly lower target for Group B Part 1 (Dynamic). Shows issue with balancing base difficulty for veterans. \\ % (b) 
            \textbullet\space Low standard deviation in all observed samples. \\ % (sd)
        } \\
        % ----------------------------------------------------------------
        % Attack avoidance efficiency
        \makecell[c]{Attack\\Avoidance\\Efficiency} & 
        \makecell*[{{p{33em}}}]
        {
            \textbullet\space Mixed results between parts of experiment for both groups. \\ % (global) 
            \textbullet\space Overall lower efficiency in Group A in both parts, comparing to Group B. Could be related to individual differences in player skill. Shows issues with sampling. \\ % (c,d)
            \textbullet\space High standard deviation in a considerable amount of samples. \\ % (sd) 
        } \\
        % ----------------------------------------------------------------
        % Attack Window Efficiency
        \makecell[c]{Attack\\Window\\Efficiency} & 
        \makecell*[{{p{33em}}}]
        {
            \textbullet\space Higher efficiency in \nth{2} part during first levels for both groups. Mixed results in later levels (4 and 5). Shows difficulty spike. \\ % (global) 
            \textbullet\space Slightly higher efficiency in Group B Part 1 (Dynamic), comparing to Group A. Results harmonize with adjustment targets. \\ % (c) 
            \textbullet\space Mixed results between groups in the \nth{2} part. \\ % (d) 
            \textbullet\space High standard deviation in a considerable amount of samples for part 2. \\ % (sd) 
        } \\
        % ----------------------------------------------------------------
        % Completion time
        \makecell[c]{Completion\\Time} & 
        \makecell*[{{p{33em}}}]
        {
            \textbullet\space Globally lower completion times in \nth{2} playthrough. \\ % (global) 
            \textbullet\space Steady increase in completion times at each level. \\ % (a) 
            \textbullet\space Overall lower completion times in Group B Part 1 (Dynamic) in comparison to Group A. Possibly caused by lower average difficulty. \\ % (c) 
            \textbullet\space Abnormally low completion times and standard deviation in some of the samples. Veteran players possibly ignored enemies. \\ % (d) 
            \textbullet\space Abnormally high standard deviation in last level. \\ % (sd) 
        } \\
        % ----------------------------------------------------------------
        ...
        \makecell*[{{p{33em}}}]
        {
            \textbullet\space Mixed results in damage dealt between playthroughs for both groups. Reinforces issues with balancing for Veteran players. \\ % (global) 
            \textbullet\space Slightly higher damage dealt in Group B \nth{1} part (Dynamic). Harmonizes with lower difficulty. \\ % (c) 
            \textbullet\space Mixed results between groups for \nth{2} part. \\ % (d) 
            \textbullet\space High standard deviation in a considerable amount of samples. \\ % (sd) 
        } \\
        % ----------------------------------------------------------------
        % Deaths per Level
        ...
        \makecell*[{{p{33em}}}]
        {
            \textbullet\space Globally lower amount of deaths in \nth{2} playthrough. \\ % (global) 
            \textbullet\space Zero deaths in second part for both groups.  \\ % (a) 
            \textbullet\space High amount of deaths in last level for both groups. Possible difficulty spike. \\ % (b) 
            \textbullet\space Slightly lower amount of deaths in Group B Part 1 (Dynamic), which aligns with lower adjustment targets.\\ % (c) 
            \textbullet\space Mixed results between groups in the second playthrough. \\ % (d) 
            \textbullet\space High standard deviation in a considerable amount of samples. \\ % (sd) 
        } \\
        % ----------------------------------------------------------------
        % Health Lost per Encounter
        ...
        \makecell*[{{p{33em}}}]
        {
            \textbullet\space Globally lower average health lost in \nth{2} playthrough. \\ % (global)
            \textbullet\space Slightly lower amount of health lost in Group B in \nth{1} playthrough (Dynamic). Harmonizes with lower adjustment targets. \\ % (c) 
            \textbullet\space Mixed results between groups in \nth{2} playthrough. Shows issues with balancing and fixed presets. \\ % (d) 
            \textbullet\space High standard deviation in some of the samples. \\ % (sd) 
        } \\
        % ----------------------------------------------------------------  
        \bottomrule
      \end{tabular}
...
% (global)  Globally lower adjustment targets in the Dynamic difficulty version for both Groups. Might show issue with balancing and target thresholds.
...
% (Dynamic) Significantly lower target for Group B Part 1.
% (b) Shows issue with balancing base difficulty for veterans. 
% (sd) Low standard deviation in all observed samples.
...
% (global) Mixed results between parts of experiment for both groups.
% (c,d) Overall lower efficiency in Group A in both parts, comparing to Group B. Could be related to individual differences in player skill. Shows issues with sampling.
% (sd) High standard deviation in a considerable amount of samples.
...
% (global) Higher efficiency in \nth{2} part during first levels for both groups. Mixed results in later levels (4 and 5). Shows difficulty spike.
...
% (c) Slightly higher efficiency in Group B Part 1 (Dynamic), comparing to Group A. Results harmonize with adjustment targets. 
% (d) Mixed results between groups in the \nth{2} part.
% (sd) High standard deviation in a considerable amount of samples for part 2.
...
% (global) Globally lower completion times in \nth{2} playthrough.
% (a) Steady increase in completion times at each level. 
% (c) Overall lower completion times in Group B Part 1 (Dynamic) in comparison to Group A. Possibly caused by lower average difficulty.
% (d)  Abnormally low completion times and standard deviation in some of the samples. Veteran players possibly ignored enemies.
% (sd) Abnormally high standard deviation in last level.
...
% (global) Mixed results in damage dealt between playthroughs for both groups. Reinforces issues with balancing for Veteran players.
% (c) Slightly higher damage dealt in Group B \nth{1} part (Dynamic). Harmonizes with lower difficulty.
% (d) Mixed results between groups for \nth{2} part.
% (sd) High standard deviation in a considerable amount of samples.
...
% (global) Globally lower amount of deaths in \nth{2} playthrough.
% (a) Zero deaths in second part for both groups.
% (b) High amount of deaths in last level for both groups. Possible difficulty spike.
...
% (c) Slightly lower amount of deaths in Group B Part 1 (Dynamic), which aligns with lower adjustment targets.
% (d) Mixed results between groups in the second playthrough.
% (sd) High standard deviation in a considerable amount of samples.
...
% (global) Globally lower average health lost in \nth{2} playthrough.
% (c) Sligthly lower amount of health lost in Group B in \nth{1} playthrough (Dynamic). Harmonizes with lower adjustment targets. \\ 
% (d) Mixed results between groups in \nth{2} playthrough. Shows issues with balancing and fixed presets.
% (sd) High standard deviation in some of the samples.
...
% 80% disagreed the game to be difficult
% None found the difficulty to be unfair
...
% 70% found the enemies to be challenging
% None found the enemies to be too challenging for them
% Mixed results on enemies being too challenging for them, with most disagreeing
% All players found the enemies in later levels to be harder
...
% 90% felt like they learned how to defend against enemy attacks
% 80% learned how to dodge
% all players found themselves constantly outnumbered
...
% 70% of players disagreed that the second version was easier than the first
% All players thought the second version was harder than the first
...
% Most players did not know if enemies in the second version were more or less intelligent
...
% Objectives of determining if
%	- Dynamic first can better understand game mechanics and enemy patterns
%	- Dynamic on second provides a more appropriate level of challenge
...
%	- Hardware requirements:
%		- To select users that could run the application with a standardized experience
%		- Without low framerates
%		- Using the same input devices
%		- Using the same display method
...
% User classification
%	- To evaluate the impact of adjustments in each player profile
%	- To evaluate the balancing of the game for each target skill level
%	- Separate users in beginners, intermediate, veterans
%		- Based on affinity to action games, weekly playtime and completion of souls-likes
...
% Performance Metrics
%	- To evaluate player performance for each group and classification
%	- To evaluate the level of challenge experienced by each group
%	- Based on:
%		- Effectiveness to complete levels
%		- Offensive efficiency 
%		- Defensive efficiency
%		- Adjustment targets (defines average difficulty)
...
%	- To detect issues with our application that could skew performance metrics
%	- To validate the difficulty balancing of the game for each group
%	- To validate the learning curve for each group
%	- To assess performance metrics in comparison to player perceptions
%	- Perception Survey
%	- Likert Scale
We also defined and employed a Player Perception Survey to assess the player perception, with the following objectives:
\begin{itemize}
...
% Summary
% - Perception survey for all users
% - Performance & perception of beginner, intermediate, veteran players

% Perception survey for all users
% =======================

% Aesthetics
% - Overwhelmingly majority of players were interested in the game, and found the game to be visually appealing
% - Players also found the game satisfying to play.
% - Most players found the audio to be satisfactory
% - Aesthetics of the game made players generally interested and motivated to play.
...
% Ease of learning basic controls
% - Majority found the game to be simple to understand and to play.
% - Divisive results regarding the game being easy to learn, with a slightly % positive opinion.
% - There were possible issues with the learning curve or specific game mechanics.
...
% Issues when playing
% - Players did not find significant issues with basic controls.
% - Problem with formulation of questions on issues regarding hitting enemies, % blocking and dodging. Players may have found propositions to be related to game % difficulty.
% - No relevant issues regarding players being lost on in-game environments due to % poor level layouts.
...
% Feelings when playing
% - Players did not feel generally frustrated, anxious or bored while playing.
% - Issues when capturing affect-based metrics due to short duration of the experiment (30 minutes)
% - Mixed results on feeling comfortable, which could be due to difficulty.
Regarding player opinions on affective reactions when playing, players did not feel generally frustrated, anxious or bored while playing. Mixed results were found regarding being comfortable when playing the game, which could be caused by game difficulty or the experiment methodology. Additionally, we identified another issue with the design of our methodology, where a short experiment duration would not be sufficient for players to correctly evaluate their perceptions on feelings.
% ! Conclusions: short experiment duration would not be sufficient for players to correctly evaluate their perceptions on feelings


% Performance of beginner players
% =======================

% Adjustment targets
% - Slightly higher difficulty on dynamic first playthrough for beginners
% - Higher difficulty in second playthrough for dynamic difficulty. 
% - Adjustments seem better suited to player's learning curve
% - Difficulty in second playthrough seem to be a better suited level of challenge
...
% - Slightly higher offensive and defensive efficiency throughout levels for % players which experienced Dynamic difficulty first
% - Overall higher offensive and defensive efficiency from players which % experienced fixed difficulty in second playthrough
% - Beginner Players which experienced dynamic difficulty in first playthrough % were more efficient
...
% - Beginner players which experienced dynamic difficulty in second playthrough % were more challenged
% Completion times
% - Follows similar results to offensive and defensive efficiency
We conclude that the difference in performance on a second playthrough was caused by players in dynamic difficulty experienced a significantly increased level of challenge. Completion times followed the same trend as combat efficiency, where players which experienced dynamic difficulty in a first playthrough were more time efficient, and in a second playthrough experienced a higher level of challenge.
...
% ! Conclusions: 
% ! - slight indications Dynamic Difficulty provided a better learning curve for beginners
% ! - strong indications that Dynamic Difficulty provided a more appropriate level of challenge on a second playthrough

% Perception survey for beginner players
% =======================

% Difficulty:
% - Most beginner players perceived the game to be difficult without being unfair, % with enemies being perceived as the appropriate level of challenge.
% - Overall, we had indications that players felt they experienced the correct % level of challenge, but a more granular analysis could expose issues with the % level of challenge.
...
% ! Conclusions: Strong indications that beginners experienced correct level of challenge

% Learning
% - Most users agreed to have learned how to block
% - Most disagreed to have learned how to dodge
% - Almost all players agreed to feeling constantly outnumbers
% - Issues with learning the dodging and isolation mechanics
...
% ! Conclusions: Issues with beginners learning dodging and target isolation

% Playthroughs
% - Group A correctly perceived the difficulty of Dynamic version to be harder, % but also felt more confident in their second playthrough
% - Group B incorrectly perceived the second version to be harder or could not % differentiate. Even though their performance was higher than Group A
% - Issue with beginners assessing the difficulty
Regarding beginner player perceptions on each playthrough, Group A correctly perceived the difficulty of the second playthrough (Dynamic) to be harder, but also felt more confident in such playthrough. Group B incorrectly assessed the second version to be harder, even though their performance in such version was much higher than Group A. We conclude that either beginner players had issues when assessing the difficulty, or the application contains balancing issues for beginner players.
% ! Conclusions: Beginners had issues assessing difficulty, or balancing issues for beginners


% Performance of intermediate players
% =======================

%Adjustment targets
%- Significant increase in targets for second part
%- Decrease in first part
%- DDA for second part is a more appropriate challenge level
%- Fixed difficulty presents are unbalanced for intermediates
%	- Created issue with the learning curve
...
% Offensive and defensive efficiency
% - Overall Higher efficiency in Dynamic First part, better learning curve
% - Overall higher efficiency in Fixed in second part, due to lower difficulty
% - Intermediate difficulty has significant difficulty spikes, which affected % performance
% - Learning curve for dynamic also makes intermediate players more efficient
% - Level of challenge is also higher 
% - Standard deviation issues
% Completion times
% - Follow the same trend as combat efficiency
...
% Perception survey for intermediate players
% =======================

%Difficulty:
%- Almost all players found the game to be difficult but not unfair. Most players %strongly agreed that enemies were challenging, but not too challenging for their %skill level.
%- Results indicate that the difficulty is appropriately balanced towards %intermediate players.
...
%Learning:
%- Most players agreed to have learned how to block, but dodging had mixed %results. Almost all felt to be constantly outnumbered.
%- Results show some improvements on the dodging mechanic when compared to %beginner players. Further evidence regarding issues with the base learning curve.
Regarding learning basic game mechanics, most players agreed to have learned how to block, but the proposition regarding the \emph{dodging} mechanic presented mixed results. Additionally, almost all players felt that they were constantly outnumbered. We conclude that results showed some improvements on the dodging mechanic when compared to beginner players, but were still suboptimal. Such results reinforce previous evidence regarding the base learning curve for each skill level.

%Playthroughs
%- Group A incorrectly assessed the second playthrough to be harder than the first
%- Group B incorrectly assessed that the second playthrough was easier than the %first
%- Issues with players identifying difficulty
%- Issues with balancing difficulty for adjustment targets
...
% Performance of veteran players
% =======================

% Adjustment targets
% - Overall lower adjustment targets for Dynamic Difficulty during both parts of % the experiment
% - Even in the second playthrough adjustment targets were significantly lower % than the fixed presets
% - Shows issue with the adjustment targets balancing for veterans
% - Issue with not having a higher difficulty than the preset for veteran players. % Adjustment targets can only go down
...
Additionally, such results also provide strong indications that a higher difficulty preset would be necessary, where players which performed significantly well would experience higher adjustment targets for a subset of the in-game adjustments.

% Offensive and defensive efficiency
% - Mixed results in during both playthroughs
% - After a certain level of difficulty, adjustment targets would not have a % significant effect on player performance
...
% - Player classification for veterans does not take into account individual % player differences
% - High standard deviation issue
% Completion times
% - Player on Dynamic Difficulty had lower times to complete levels on both % playthroughs
% - Could be explained by the lower average difficulty
...
% Perception survey for veteran players
% =======================

% Difficulty:
% - Almost all veteran players disagreed that the game was difficulty, but most % thought the enemies were challenging. Some of the players found enemies to be % too easy for them. 
% - Such results further indicate issues with the adjustment targets, where even % performing well the game lowers difficulty
% - Veteran players were not able to determine which of the versions was more % difficult
% - Such results align with the adjustment targets, which provided a similar % difficulty level
Regarding the perceptions of difficulty by veteran players, almost all players disagreed that the game was difficult, but most thought that the enemies were challenging. A small amount of players found enemies to be too easy for their skill level. Such results further indicate issues with adjustment targets, where even when player perform well, the game lowers the average difficulty from adjustment targets.
...
% Learning
% - Most players reported to have learned how to defend and dodge against enemy % attacks. 
% - The results for the dodging mechanic go in contrast with beginners and % intermediates
% - However, all players felt to be constantly outnumbered. 
% - Global issue with the target isolation mechanic, which could be explained by a % failure in level design
...
In this section, we describe the problems and limitations observed with the design of our experiment methodology and the analysis of results. We provide no structured or ordered approach to the description of issues encountered. Instead, we attempt to list such issues as they were encountered during the process of analyzing results from the experiment.

% High std deviation in significant amount of metrics
...
% Because veterans and beginners should be able to extrapolate higher and lower limits.
